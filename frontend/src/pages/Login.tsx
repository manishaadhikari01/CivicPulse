  ...
  return (
    <div className="mx-auto max-w-md space-y-6 px-4 py-8">
      <div>
        ...
        <p className="text-sm text-on-surface/60">Sign in to track reports and earn civic XP.</p>
      </div>
      <form onSubmit={submit} className="space-y-4 rounded-2xl border border-outline/15 bg-white p-6 shadow-sm">
        ...
        <label className="block space-y-1 text-sm">
          ...
          <input type="password" required value={password} onChange={(e) => setPassword(e.target.value)} className="w-full rounded-lg border border-outline/30 px-3 py-2.5" />
        </label>
      ...
      <p className="text-center text-sm text-on-surface/60">
        No account? <Link to="/signup" className="font-medium text-primary">Create one</Link>
      </p>
    </div>
  )
}
